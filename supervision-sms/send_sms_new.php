        ...
        if ($row["status"] == "ACTIVE") {
            ...
            if ($result->num_rows > 0) {
                while ($row = $result->fetch_assoc()) {
                    ...
                    if ($cradit_amount > sizeof($numberArray)) {
...
//                        if ($cradit_amount > $sms_count) {
                        for ($x = 0; $x < sizeof($numberArray); $x++) {
                            if ($sms_count == 1) {
                                ...
                                if ($response === false) {
                                    ...
                                    if ($error == "0") {
                                        ...
                                        $sql = "INSERT INTO t_message_details (`client`, `receiver_number`, `message`, `date`, `sms_count`,`characters_count`,`camp_id`,`ref_id`,`token`) VALUES ($client, '$numberArray[$x]', '$message', '$date', 1,'$character_count','$camp_id','$ref_id','$token');";
                                        if ($conn->multi_query($sql) === TRUE) {
...
//                        echo $numberArrayCountValidate;
//                        echo sizeof($numberArray);
                        if ($numberArrayCountValidate == sizeof($numberArray)) {
                            echo '0';
                        } else {
                            echo '1';
                        }
